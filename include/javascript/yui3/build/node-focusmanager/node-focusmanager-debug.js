...
if(descendants){nDescendants=descendants.size();if(nDescendants>1){for(i=0;i<nDescendants;i++){oNode=descendants.item(i);if(nFirstEnabled===-1&&!oNode.get(DISABLED)){nFirstEnabled=i;}
if(nActiveDescendant<0&&parseInt(oNode.getAttribute(TAB_INDEX,2),10)===0){nActiveDescendant=i;}
oNode.set(TAB_INDEX,-1);sID=oNode.get(ID);if(!sID){sID=Y.guid();oNode.set(ID,sID);}
...
if(nextKeyHandler){nextKeyHandler.detach();}},_preventScroll:function(event){if(scrollKeys[event.keyCode]){event.preventDefault();}},_fireClick:function(event){var oTarget=event.target,sNodeName=oTarget.get("nodeName").toLowerCase();if(event.keyCode===13&&(!clickableElements[sNodeName]||(sNodeName==="a"&&!oTarget.getAttribute("href")))){Y.log(("Firing click event for node:"+oTarget.get("id")),"info","nodeFocusManager");oTarget.simulate("click");}},_attachKeyHandler:function(){this._detachKeyHandler();var sNextKey=this.get("keys.next"),sPrevKey=this.get("keys.previous"),oNode=this.get(HOST),aHandlers=this._eventHandlers;if(sPrevKey){this._prevKeyHandler=Y.on(KEY,Y.bind(this._focusPrevious,this),oNode,sPrevKey);}
...
oNode=this._descendants.item(nActiveDescendant);if(oNode.get(DISABLED)){this._focusNext(event,nActiveDescendant);}
...
oNode=this._descendants.item(nActiveDescendant);if(oNode.get(DISABLED)){this._focusPrevious(event,nActiveDescendant);}
else{this.focus(nActiveDescendant);}}
