...
else{Y.log("Could not find parser for field "+Y.dump(field),"warn","dataschema-json");}}
...
else{Y.log("Invalid locator: "+locator,"error","dataschema-json");}}
...
else{Y.log("JSON data could not be schema-parsed: "+Y.dump(data)+" "+Y.dump(data),"error","dataschema-json");data_out.error=new Error("JSON schema parse failure");}
...
if(error){Y.log("JSON data could not be parsed: "+Y.dump(json_in),"error","dataschema-json");data_out.error=error;}}
return data_out;},_getFieldValues:function(fields,array_in,data_out){var results=[],len=fields.length,i,j,field,key,path,parser,simplePaths=[],complexPaths=[],fieldParsers=[],result,record;for(i=0;i<len;i++){field=fields[i];key=field.key||field;path=SchemaJSON.getPath(key);if(path){if(path.length===1){simplePaths[simplePaths.length]={key:key,path:path[0]};}else{complexPaths[complexPaths.length]={key:key,path:path};}}else{Y.log("Invalid key syntax: "+key,"warn","dataschema-json");}
...
else{Y.log("XML data could not be schema-parsed: "+Y.dump(data)+" "+Y.dump(data),"error","dataschema-xml");data_out.error=new Error("XML schema parse failure");}
...
else{data_out.results=data_in;Y.log("Schema resultFields property not found: "+Y.dump(schema),"warn","dataschema-array");}}
else{Y.log("Array data could not be schema-parsed: "+Y.dump(data)+" "+Y.dump(data),"error","dataschema-array");data_out.error=new Error("Array schema parse failure");}
...
else{result=null;Y.log("Unexpected type while parsing array: "+Y.dump(item),"warn","dataschema-array");}
results[i]=result;}
...
else{Y.log("Text data could not be schema-parsed: "+Y.dump(data)+" "+Y.dump(data),"error","dataschema-text");data_out.error=new Error("Text schema parse failure");}
