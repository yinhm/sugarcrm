...
Y.log('adding trigger to group: '+this.get('name'),'info','imageloader');var wrappedFetch=function(){this.fetch();};this._triggers.push(Y.on(type,wrappedFetch,obj,this));return this;},addCustomTrigger:function(name,obj){if(!name){return this;}
Y.log('adding custom trigger to group: '+this.get('name'),'info','imageloader');var wrappedFetch=function(){this.fetch();};if(Y.Lang.isUndefined(obj)){this._triggers.push(Y.on(name,wrappedFetch,this));}
...
Y.log('adding image with id: '+domId+' to group: '+this.get('name'),'info','imageloader');this._imgObjs[domId]=new Y.ImgLoadImgObj(arguments[0]);return this._imgObjs[domId];},fetch:function(){Y.log('Fetching images in group: "'+this.get('name')+'".','info','imageloader');this._clearTriggers();this._fetchByClass();for(var id in this._imgObjs){if(this._imgObjs.hasOwnProperty(id)){this._imgObjs[id].fetch();}}},_clearTriggers:function(){clearTimeout(this._timeout);for(var i=0,len=this._triggers.length;i<len;i++){this._triggers[i].detach();}},_foldCheck:function(){Y.log('Checking for images above the fold in group: "'+this.get('name')+'"','info','imageloader');var allFetched=true,viewReg=Y.DOM.viewportRegion(),hLimit=viewReg.bottom+this.get('foldDistance'),id,imgFetched,els,i,len;if(hLimit<=this._maxKnownHLimit){return;}
...
if(els[i].y&&els[i].y<=hLimit){els[i].el.removeClass(this._className);els[i].fetched=true;Y.log('Image with id "'+els[i].el.get('id')+'" is within distance of the fold. Fetching image. (Image registered by class name with the group - may not have an id.)','info','imageloader');}
else{allFetched=false;}}}
if(allFetched){Y.log('All images fetched; removing listeners for group: "'+this.get('name')+'"','info','imageloader');this._clearTriggers();}},_fetchByClass:function(){if(!this._className){return;}
Y.log('Fetching all images with class "'+this._className+'" in group "'+this.get('name')+'".','info','imageloader');Y.all('.'+this._className).removeClass(this._className);}};Y.extend(Y.ImgLoadGroup,Y.Base,groupProto);Y.ImgLoadImgObj=function(){Y.ImgLoadImgObj.superclass.constructor.apply(this,arguments);this._init();};Y.ImgLoadImgObj.NAME='imgLoadImgObj';Y.ImgLoadImgObj.ATTRS={domId:{value:null,writeOnce:true},bgUrl:{value:null},srcUrl:{value:null},width:{value:null},height:{value:null},setVisible:{value:false},isPng:{value:false},sizingMethod:{value:'scale'},enabled:{value:'true'}};var imgProto={_init:function(){this._fetched=false;this._imgEl=null;this._yPos=null;},fetch:function(withinY){if(this._fetched){return true;}
...
Y.log('Fetching image with id "'+this.get('domId')+'".','info','imageloader');if(this.get('bgUrl')!==null){if(this.get('isPng')&&Y.UA.ie&&Y.UA.ie<=6){el.setStyle('filter','progid:DXImageTransform.Microsoft.AlphaImageLoader(src="'+this.get('url')+'", sizingMethod="'+this.get('sizingMethod')+'", enabled="'+this.get('enabled')+'")');}
