...
return n;},_finish=function(id){Y.log("Finishing transaction "+id,"info","get");var q=queues[id],msg,sc;if(q.timer){clearTimeout(q.timer);}
...
_end(id,msg,'OK');},_timeout=function(id){Y.log("Timeout "+id,"info","get");var q=queues[id],sc;if(q.onTimeout){sc=q.context||q;q.onTimeout.call(sc,_returnData(q));}
_end(id,'timeout','timeout');},_next=function(id,loaded){Y.log("_next: "+id+", loaded: "+(loaded||"nothing"),"info","get");var q=queues[id],msg,w,d,h,n,url,s;if(q.timer){clearTimeout(q.timer);}
...
url=q.url[0];if(!url){q.url.shift();Y.log('skipping empty url');return _next(id);}
Y.log("attempting to load "+url,"info","get");if(q.timeout){q.timer=setTimeout(function(){_timeout(id);},q.timeout);}
...
Y.log("Appending node: "+url,"info","get");if((ua.webkit||ua.gecko)&&q.type==="css"){_next(id,url);}},_autoPurge=function(){if(purging){return;}
...
setTimeout(function(){_next(id);},0);return{tId:id};},_track=function(type,n,id,url,win,qlength,trackfn){var f=trackfn||_next;if(ua.ie){n.onreadystatechange=function(){var rs=this.readyState;if("loaded"===rs||"complete"===rs){Y.log(id+" onreadstatechange "+url,"info","get");n.onreadystatechange=null;f(id,url);}};}else if(ua.webkit){if(type==="script"){n.addEventListener("load",function(){Y.log(id+" DOM2 onload "+url,"info","get");f(id,url);});}}else{n.onload=function(){Y.log(id+" onload "+url,"info","get");f(id,url);};n.onerror=function(e){_fail(id,e+": "+url);};}};return{PURGE_THRESH:20,_finalize:function(id){Y.log(id+" finalized ","info","get");setTimeout(function(){_finish(id);},0);},abort:function(o){var id=(L.isString(o))?o:o.tId,q=queues[id];if(q){Y.log("Aborting "+id,"info","get");q.aborted=true;}},script:function(url,opts){return _queue("script",url,opts);},css:function(url,opts){return _queue("css",url,opts);}};}();})();},'3.0.0');